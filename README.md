...
└── derivatives/
    ...
    ├── weights_and_gaze/  # output of step 5.5 (for analysis.py)
...
Both files are written to `derivatives/weights_and_gaze/` (configured via `WEIGHTS_DIR` in `config.py`) when Step 5.5 is run.
...
run_remodnav.sh                    (Step 0 — skip if using pre-classified data)
         ↓
remodnav_frame_selection.py        (Step 1)
         ↓
frames_to_vgg19features.py         (Step 2)
         ↓
remodnav_save_features_by_subs.py  (Step 3)
         ↓
form_and_save_hyperlayers.py       (Step 4)
         ↓
precision_*.py                     (Step 5 — encoding models)
         ↓
generate_weights_and_gaze.py       (Step 5.5 — required before analysis.py)
         ↓
analysis.py
```
...
---
### Step 5.5 — Generate weights and gaze distributions
**Script:** `generate_weights_and_gaze.py`

Computes the two `.pkl` files needed by `analysis.py` for Figures 3a–d from outputs already on disk. Must be run after `precision_baseline.py` (Step 5) has completed for all 13 subjects.

```bash
python generate_weights_and_gaze.py
```

**What it computes:**

- **Spatial weight maps.** The baseline encoder weights saved by `precision_baseline.py` have shape `(164864, 19629)` — a flat `(7×16×1472, N_voxels)` matrix. The script reshapes this to `(1472, 7, 16, 19629)` and takes the L2 norm over the channel dimension, collapsing to `(7, 16, 19629)`. This gives the spatial weight importance map — how much each location in the feature grid contributes to each voxel's encoding — which is used in Figures 3a, 3b, and 3d.

- **Gaze density maps.** Fixation pixel coordinates from the per-subject `.npy` files (Step 1 output) are binned onto the same 7×16 grid used during feature extraction, accumulated across all 8 runs, and normalised to a probability distribution. Stored as `(16, 7)` (x-major) so that `.T` gives a `(7, 16)` display-ready array, matching the convention used throughout `analysis.py`.

**Inputs:**
- `derivatives/results/{sub}_hyperlayer_fast_baseline_weights.npy` — saved by `precision_baseline.py`
- `derivatives/fixations/{sub}-run{run}.npy` — saved by `remodnav_frame_selection.py`

**Outputs** (`derivatives/weights_and_gaze/`)**:**
- `all_subs_weights_over_space.pkl` — list of 13 arrays, each `(7, 16, 19629)`
- `all_subs_gaze_dists.pkl` — list of 13 arrays, each `(16, 7)`
...
Produces all main and supplementary figures from the paper. Run after completing the full pipeline (Steps 1–5.5):
...
- Weights and gaze pkl files in `derivatives/weights_and_gaze/` — generated by `generate_weights_and_gaze.py` (Step 5.5); see [Weights and gaze distributions](#weights-and-gaze-distributions----derivativesweights_and_gaze) above
- 
---
