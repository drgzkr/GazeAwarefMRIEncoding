...
from config import VGG_BY_SUB_DIR, HYPERLAYERS_DIR
...
for sub in tqdm(sub_list):
    ...
    for layer in ['0','1','2','3','4']:
        sub_features = np.load(os.path.join(VGG_BY_SUB_DIR, 'Model_Features_'+sub+'_concat_run_layer'+layer+'_7_16.npy'))
        sub_hyperlayer_features[:,zero_sub_feature_sizes[int(layer)]:zero_sub_feature_sizes[int(layer)+1],:,:] = sub_features
    
    np.save(os.path.join(HYPERLAYERS_DIR, 'Model_features_'+sub+'_hyperlayer_7_16.npy'), sub_hyperlayer_features, allow_pickle=True)
