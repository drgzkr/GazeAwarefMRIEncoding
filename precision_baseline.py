...
gammas = []
for sub in tqdm(sub_list,position=0):
    ...
    encoder_model_brain_weights = np.zeros((sub_movie_data.shape[0],164864)) #7*16*1472

    # GRAB THE ENTIRE FEATURE MAP
    voxel_feature_timeseries = sub_features[np.linspace(0,sub_features.shape[0]-1,sub_features.shape[0]).astype(int),:,:,:]
    voxel_feature_timeseries = np.reshape(sub_features,newshape=(sub_features.shape[0],164864)) #7*16*1472
    ...
    learned_ws = fastRidge.get_weights()
    np.save('/home/djaoet/wrkgrp/Dora/Study_Forrest/FastBaselineResults/'+str(sub)+'_hyperlayer_fast_7gammae7_baseline_trained_feature_weights',learned_ws,allow_pickle=True)

    print('weights shape: ',learned_ws.shape)
    ...
    np.save('/home/djaoet/wrkgrp/Dora/Study_Forrest/FastBaselineResults/'+str(sub)+'_hyperlayer_fast_baseline_results',results,allow_pickle=True)
np.save('/home/djaoet/wrkgrp/Dora/Study_Forrest/FastBaselineResults/'+str(sub)+'_hyperlayer_fast_baseline_gammas',gammas,allow_pickle=True)
